...
async def on_message(message):
    ...
    if message.content in blacklist:
        embed = discord.Embed(title="NO NFT ADVERTISEMENTS ALLOWED",
                              description=f"HEY {member.mention} NO NFT ADVERTISEMENTS IS ALLOWED HERE. NFTS are causing harm to the world. Learn more at nft!nfttruth", color=(40959))
        embed.add_field(
            name="CASE DETAILS", value=f'NFT Advertiser Mention: {member.mention}\nNFT Advertising Type: NFT Website.', inline=False)
...
async def help(ctx):
    embed = discord.Embed(
        title="ANTI-NFT Bot", description="Here is my entire Commands List", color=(40959))
    embed.add_field(name="GENERAL COMMANDS",
                    value='nft!help - This message\nnft!ping - Checks my Latency\nnft!info - Information about the Bot\nnft!privacy - Checks my privacy Policy', inline=False)
    embed.add_field(
        name="ANTI-NFT", value='nft!nfttruth - Tells you the truth about NFTs', inline=False)
...
async def info(ctx):
    embed = discord.Embed(
        title="ANTI-NFT Bot", description="Information about the NFT discord bot.", color=(40959))
    embed.add_field(
        name="GENERAL", value='CREATOR: Idiot Creature Hater#2255', inline=False)
...
async def privacy(ctx):
    embed = discord.Embed(title="ANTI-NFT Bot Privacy Policy",
                          description="Last updated: July 4, 2022", color=(40959))
    ...
    embed.add_field(name="HOW MAY I CONTACT YOU FOR CONCERNS",
                    value='Reach to use at our support server at: https://discord.gg/NpRbKaFmJU', inline=False)
    await ctx.send(embed=embed)
...
async def nfttruth(ctx):
    embed = discord.Embed(
        title="ANTI-NFT Bot", description="Here is the truth about NFTs", color=(40959))
    embed.add_field(name="NFTs CAUSES HARM TO THE ENVIRONMENT",
                    value='NFTs are making a bad impact to the world. Learn more about the truth of NFTs here: https://rickorford.com/8-reasons-to-not-invest-in-nfts/', inline=False)
    await ctx.send(embed=embed)
